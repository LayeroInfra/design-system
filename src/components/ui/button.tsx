...
const buttonVariants = cva(
  // Зазор между иконкой и подписью — 6px, а не 8: иконка мельче буквы, и на
  // восьми пикселях подпись читается отдельным словом, а не частью кнопки.
  "inline-flex items-center justify-center gap-1.5 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-200 ease-out select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98] [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
        ...
        outline:
          "border border-border bg-card text-foreground shadow-sm hover:bg-overlay hover:border-foreground/30",
        secondary:
